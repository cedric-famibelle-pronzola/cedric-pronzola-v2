...
Edit `.env.local` with your specific configuration. See `.env.sample` for all available variables and examples including:

* **Site Configuration**: Base URL for your deployment
* **SMTP Settings**: Email server configuration for the contact form  
...
Copiez le fichier d'exemple des variables d'environnement et configurez vos paramètres :
...
Modifiez `.env.local` avec votre configuration spécifique. Consultez `.env.sample` pour toutes les variables disponibles et des exemples incluant :

* **Configuration du Site** : URL de base pour votre déploiement
* **Paramètres SMTP** : Configuration du serveur email pour le formulaire de contact
